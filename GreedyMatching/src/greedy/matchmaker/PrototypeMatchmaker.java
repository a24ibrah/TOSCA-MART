...
public class PrototypeMatchmaker extends Matchmaker {
	 ...
	 */
	public PrototypeMatchmaker(TDefinitions nodeTypeDefs, String servTempLoc, int probability) throws TOSCADefinitionsNotFoundException, TOSCADefinitionsNotValidException {
		super(nodeTypeDefs,servTempLoc,probability);
...
//			super(nodeTypeLoc,servTempLoc);
	 ...
	 *
	 * @param args The method takes two String arguments representing the locations of the NodeType and of the ServiceTemplate to be matched, and the probability of having a ServiceTemplate's internal feature exposed on the boundaries.
	 */
	public static void main(String[] args) {
		if(args.length!=3) {
			System.err.println("The arguments to be specified are THREE.");
			return;
		}
		String nodeTypeLoc = args[0];
		String servTempLoc = args[1];
		...
		for(int run=0; run<runs; run++) {
			...
			try {
				...
				mm = new PrototypeMatchmaker(nodeTypeDefs,servTempLoc,probability);
			} catch (Exception e) {
				e.printStackTrace();
				return;
			}
...
//				System.err.println("NO CANDIDATES! {Pay attention to the fact that mappings can be empty (e.g., if the target node specifies no properties)}");
		...
		System.out.println("\n MatchingRuns = " + matchingRuns + "/" + runs);
	}
}