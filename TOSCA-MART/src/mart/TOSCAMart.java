...
import mart.mapping.CapabilityMapping;
import mart.mapping.Mapping;
import mart.mapping.OperationMapping;
import mart.mapping.PropertyMapping;
import mart.mapping.RequirementMapping;
import mart.matchmaker.PrototypeMatchmaker;
...
/**
 * This class implements the "TOSCA-MART" matchmaking and adaptation approach proposed in the
 ...
 *
 */
public class TOSCAMart {

    /**
     * This method implements the "Matchmake" step of the "TOSCA-MART" approach.
     *
     * @param nodeTypeDefs TDefinitions of the NodeType to be matched.
     * @param servTempLoc String representing the location of the ServiceTemplate to be matched.
     *
     * @return List of Candidate topology fragments.
     *
     * @throws TOSCADefinitionsNotFoundException @see TOSCADefinitionsNotFoundException
     * @throws TOSCADefinitionsNotValidException @see TOSCADefinitionsNotValidException
     */
    private static List<Candidate> matchmake(TDefinitions nodeTypeDefs, String servTempLoc) throws TOSCADefinitionsNotFoundException, TOSCADefinitionsNotValidException {
        PrototypeMatchmaker mm = new PrototypeMatchmaker(nodeTypeDefs,servTempLoc);
        return mm.run();
    }

    /**
     * This method implements the "CandidatesUnion" step of the "TOSCA-MART" approach.
     *
     * @param nodeTypeDefs TDefinitions of the NodeType to be matched.
     ...
     * @return List of Candidate topology fragments.
     */
    private static List<Candidate> candidatesUnion(TDefinitions nodeTypeDefs, List<String> servTempLocs) {
        ...
        for(String servTempLoc : servTempLocs){
            try {
                candidates.addAll(matchmake(nodeTypeDefs,servTempLoc));
            } catch (Exception e) {
                System.err.println("[CandidatesUnion]:\t" + e.getMessage());
            }
        }
        return candidates;
    }
    ...
    /**
     * This method is used to rate a Candidate.
     *
     * @param cand Candidate to be rated.
     */
    private static void rate(Candidate cand) {
        cand.setRate(1.0 / cand.getFragment().size());
    }

    /**
     * This method implements the "Rate" and "Filter" steps of the "TOSCA-MART" approach.
     *
     * @param nodeTypeDefs TDefinitions of the NodeType to be matched.
     * @param servTempLocs String(s) representing the location of the ServiceTemplate(s) to be matched.
     *
     * @return List of Candidate topology fragments.
     */
    private static List<Candidate> rateAndFilter(TDefinitions nodeTypeDefs, List<String> servTempLocs) {
    	long elapsedTime = System.nanoTime();
    	List<Candidate> candidates = candidatesUnion(nodeTypeDefs,servTempLocs);
    	elapsedTime = System.nanoTime() - elapsedTime;
//        System.err.println("[CandidatesUnion ---- Elapsed Time(micros)]:\t" + elapsedTime/1000);
        System.out.print(elapsedTime/1000 + ",");
        List<Candidate> ratedCandidates = new ArrayList<Candidate>();
        //For each candidate fragment,
        for(Candidate cand : candidates) {
            //Computes its rate
            rate(cand);
            //Properly adds "cand" to the "ratedCandidates"
            //(if not already present)
            if(ratedCandidates.isEmpty())
                ratedCandidates.add(cand);
            else {
                //Determines the position where to place "cand"
                boolean added = false;
                for(int i=0; i<ratedCandidates.size() && !added; i++) {
                    Candidate current = ratedCandidates.get(i);
                    //Checks whether "cand" is already in "ratedCandidates"
                    //(by looking whether "current" is equal to "cand"
                    if(current.getFragment().size()==cand.getFragment().size()) {
                        boolean alreadyIn = true;
                        for(TEntityTemplate eCurr : current.getFragment()) {
                            boolean isEqual = false;
                            for(TEntityTemplate eCand : cand.getFragment()) {
                            	if(eCurr.getType().equals(eCand.getType())) {
                                    isEqual = true;
                                    break;
                                }
                            }
                        ...
                        if(alreadyIn)
                            //If so, considers "cand" as already "added"
                            added = true;
                    }
                    //Otherwise, checks whether "cand" should be placed before
                    //"current"
                    else if(current.getRate()<cand.getRate()) {
                        ratedCandidates.add(i, cand);
                        added = true;
                    }
                }
                if(!added)
    ...
    /**
     * This method implements the "Cut" step of the "TOSCA-MART" approach.
     *
     * @param nodeTypeDefs TDefinitions of the NodeType to be matched.
     * @param servTempLocs String(s) representing the location of the ServiceTemplate(s) to be matched.
     * @param theta Threshold representing the maximum number of candidates to be detected.
     *
     * @return List of Candidate topology fragments.
     */
    private static List<Candidate> cut(TDefinitions nodeTypeDefs, List<String> servTempLocs, int theta) {
        long elapsedTime = System.nanoTime();
    	List<Candidate> ratedCandidates = rateAndFilter(nodeTypeDefs,servTempLocs);
    	elapsedTime = System.nanoTime() - elapsedTime;
//        System.err.println("[RateAndFilter ------ Elapsed Time(micros)]:\t" + elapsedTime/1000);
    	System.out.print(elapsedTime/1000 + ",");
        if(ratedCandidates.size()<=theta)
            return ratedCandidates;
        return ratedCandidates.subList(0, theta);
    }

    /**
     * This method implements the mapping selection policy to be employed in the "Mapping Selection" step.
     *
     * @param cand Candidate topology fragment.
     *
     * @return Candidate topology fragment (containing only the selected mapping in the set of possible mappings).
     */
    private static Candidate selectMapping(Candidate cand) {

        //=========================================
        //Weights EntityTemplate(s) in the Topology
        // (by associating them their deepness)
        //=========================================
        TDefinitions startDefs = cand.getStartingTopology();
        TTopologyTemplate top = null;
        //Takes the TopologyTemplate from which the fragment is excerpted
        for(TExtensibleElements elem : startDefs.getServiceTemplateOrNodeTypeOrNodeTypeImplementation())
        	if(elem instanceof TServiceTemplate)
        		top = ((TServiceTemplate) elem).getTopologyTemplate();
        //Builds lists of NodeTemplates and RelationshipTemplates composing the service topology
        List<TNodeTemplate> nodeTemps = new ArrayList<TNodeTemplate>();
		...
		for(TEntityTemplate entity : top.getNodeTemplateOrRelationshipTemplate()){
			if(entity instanceof TNodeTemplate)
				nodeTemps.add((TNodeTemplate) entity);
			if(entity instanceof TRelationshipTemplate)
				relTemps.add((TRelationshipTemplate) entity);
		}
		//Builds a HashMap ("deepnessMap") associating a "deepness" to each EntityTemplate in "top"
		// (as follows:
		//    deepness = 0;
		//    repeat
		//      detect the set N of nodes without incoming dependencies
		//      for each node n in N, adds (n,deepness) to deepnessMap
		//      removes the nodes in N from those to be considered
		//      detect the set R of relationship starting from a node in N
		//      for each relationship r in R, adds (r,deepness+1) to deepnessMap
		//      removes the relationships in R from those to be considered
		//    until (no more nodes have to be considered)
		HashMap<TEntityTemplate,Integer> deepnessMap = new HashMap<TEntityTemplate,Integer>();
		int deepness = 0;
		while(!nodeTemps.isEmpty()) {
			for(int i=nodeTemps.size()-1; i>=0; i--) {
				TNodeTemplate n = nodeTemps.get(i);
				boolean isRoot = true;
				for(TRelationshipTemplate r : relTemps) {
					TEntityTemplate target = (TEntityTemplate) r.getTargetElement().getRef();
					//If "source" is a requirement
                    if(target instanceof TRequirement) {
                        TRequirement targetReq = (TRequirement) target;
                        //stores the NodeTemplate in which "sourceReq" appears as the
                        //"source" of the relationship
                        for(TEntityTemplate e : top.getNodeTemplateOrRelationshipTemplate()) {
                        	if(e instanceof TNodeTemplate) {
                        		TNodeTemplate eNode = (TNodeTemplate) e;
                        		if(eNode.getRequirements() != null)
                        			if(eNode.getRequirements().getRequirement().contains(targetReq))
                        				target = eNode;
                        	}
                        }
                    }
				...
				if(isRoot) {
					deepnessMap.put(n, new Integer(deepness));
					nodeTemps.remove(n);
				}
			}
			for(int j=relTemps.size()-1; j>=0; j--) {
				TRelationshipTemplate r = relTemps.get(j);
				if(deepnessMap.containsKey(r.getSourceElement().getRef())) {
					deepnessMap.put(r, new Integer(deepness+1));
					relTemps.remove(r);
				}
			}
			deepness = deepness + 2;
        ...
        //=========================================
        //Computes the "best" mapping
		// (by selecting the one which leads to the
		//  minimum sum of deepness values)
        //=========================================
		int minimumSum = -1;
		int currentSum;
		...
		for(Mapping m : cand.getMappings()) {
			...
			for(CapabilityMapping cm : m.getCapsMap())
				currentSum += deepnessMap.get(cm.getServCap().getNodeTemplate()).intValue();
			for(RequirementMapping rm : m.getReqsMap())
				currentSum += deepnessMap.get(rm.getServReq().getNodeTemplate()).intValue();
			for(PropertyMapping pm : m.getPropsMap())
				currentSum += deepnessMap.get(pm.getServProp().getEntityTemplate()).intValue();
			for(OperationMapping om : m.getOpsMap())
				currentSum += deepnessMap.get(om.getServOp().getEntityTemplate()).intValue();
    ...
    /**
     * This method implements the "Mapping Selection" step of the "TOSCA-MART" approach (by employing the selection policy defined with the method "selectMapping").
     *
     * @param nodeTypeDefs TDefinitions of the NodeType to be matched.
     * @param servTempLocs String(s) representing the location of the ServiceTemplate(s) to be matched.
     * @param theta Threshold representing the maximum number of candidates to be detected.
     ...
     * @return List of Candidate topology fragments.
     */
    private static List<Candidate> mappingSelection(TDefinitions nodeTypeDefs, List<String> servTempLocs, int theta) {
        long elapsedTime = System.nanoTime();
    	List<Candidate> ratedCandidates = cut(nodeTypeDefs,servTempLocs,theta);
    	elapsedTime = System.nanoTime() - elapsedTime;
//    	System.err.println("[Cut ---------------- Elapsed Time(micros)]:\t" + elapsedTime/1000);
    	System.out.print(elapsedTime/1000 + ",");
    ...
    /**
     * This method implements the "Adaptation" step of the "TOSCA-MART" approach.
     *
     * @param nodeTypeLoc String representing the location of the NodeType to be matched.
     * @param servTempLocs String(s) representing the location of the ServiceTemplate(s) to be matched.
     * @param theta Threshold representing the maximum number of candidates to be detected.
     *
     * @return List of TDefinitions, each of which is a possible implementation of the target NodeType.
     */
    private static List<TDefinitions> adaptation(String nodeTypeLoc, List<String> servTempLocs, int theta) {
...
//        System.err.println("[Adaptation - Initial Parsing Time(micros)]:\t0"+parsingTime/1000);

        long elapsedTime = System.nanoTime();
        List<Candidate> electedCandidates = mappingSelection(nodeTypeDefs, servTempLocs, theta);
    	elapsedTime = System.nanoTime() - elapsedTime;
//    	System.err.println("[MappingSelection --- Elapsed Time(micros)]:\t" + elapsedTime/1000);
    	System.out.print(elapsedTime/1000 + ",");
        ...
        for(Candidate cand : electedCandidates) {
            ...
            TDefinitions startDefs = cand.getStartingTopology();
            //Creates a new id
            candDefs.setId(startDefs.getId() + "_Adaptation" + candNumber);
    ...
    /**
     * This method operates the "TOSCA-MART" matchmaking and adaptation method.
     *
     * @param nodeTypeLoc TDefinitions of the NodeType to be matched.
     * @param servTempLocs String(s) representing the location of the ServiceTemplate(s) to be matched.
     * @param howManyImplementations Threshold representing the maximum number of candidates to be detected.
     *
     * @return List of String(s) containing the names of generated TOSCA specification files.
     */
    public static List<String> run(String nodeTypeLoc, List<String> servTempLocs, int howManyImplementations) {
    	//Generates the "adaptedServices" by invoking the "Adaptation" step of "TOSCA-MART"
    	long elapsedTime = System.nanoTime();
    	List<TDefinitions> adaptedServices = adaptation(nodeTypeLoc, servTempLocs, howManyImplementations);
    	elapsedTime = System.nanoTime() - elapsedTime;
//        System.err.println("[Adaptation --------- Elapsed Time(micros)]:\t" + elapsedTime/1000);
    	System.out.print(elapsedTime/1000 + ",");

    	//Generates (and returns) the list of String(s) containing the names of the generated specs
    	List<String> generatedSpecs = new ArrayList<String>();
    	for(TDefinitions adaptedDefs : adaptedServices) {
    		String specFileName = ((TServiceTemplate) adaptedDefs.getServiceTemplateOrNodeTypeOrNodeTypeImplementation().get(0)).getName() + ".tosca";
    		TOSCAUtils.writeDefinitions(adaptedDefs, specFileName);
    		generatedSpecs.add(specFileName);
    	}
    ...
    public static void main(String[] args) {
        ...
        int howManyImplementations = Integer.parseInt(args[2]);
        //Generates specs of adapted services
        long elapsedTime = System.nanoTime();
        List<String> adaptedSpecs = run(nodeTypeLoc,servTempLocs,howManyImplementations);
        elapsedTime = System.nanoTime() - elapsedTime;
        System.out.println("\n Generated specification(s): [" + adaptedSpecs.size() + "/" + howManyImplementations + "]");
        for(String as : adaptedSpecs)
        	System.out.println("|_ " + as);
        System.out.println("\n Elapsed Time(micros): " + elapsedTime/1000);
    }
}
