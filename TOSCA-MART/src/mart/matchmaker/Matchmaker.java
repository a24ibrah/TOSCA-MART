...
import mart.mapping.mts.MTCapability;
...
public abstract class Matchmaker {
	 ...
	 */
	public Matchmaker(TDefinitions nodeTypeDefs, String servTempLoc) throws TOSCADefinitionsNotFoundException, TOSCADefinitionsNotValidException {
	...
	protected Set<List<CapabilityMapping>> matchCapabilities(
		...
		//Checks whether there are no more needed capabilities
	...
	private Set<List<RequirementMapping>> matchRequirements(
		...
		//Checks whether there are no more needed requirements
		boolean isSolution = true;
		for(TRequirementDefinition cr : needed) {
			if(cr.getLowerBound()!=0)
				isSolution=false;
		}
		if(isSolution) {
			//If so, adds "matched" to "solutions"
		...
		//Checks whether there are no more available requirements
		if(available.size()==0) {
			//If so, returns the empty set
			return solutions;
		}
	...
	protected Set<List<PropertyMapping>> matchProperties(
		...
		//Checks whether there are no more needed properties
		if(needed.size()==0) {
			//If so, adds "matched" to "solutions"
			//  If "needed" is initially empty, the added "matched" is empty and
			//  represents that no mappings have been/should be detected.
		...
		//Checks whether there are no more available properties
		if(available.size()==0) {
			//If so, returns the empty set
			return solutions;
		}
	...
	private Set<List<OperationMapping>> matchOperations(
		...
		//Checks whether there are no more needed operations
		if(needed.size()==0) {
			//If so, adds "matched" to "solutions"
			//  If "needed" is initially empty, the added "matched" is empty and
			//  represents that no mappings have been/should be detected.
	...
	public List<Candidate> run() {
		...
		for(TEntityTemplate entity : top.getNodeTemplateOrRelationshipTemplate()){
			...
			if(entity instanceof TRelationshipTemplate) {
				...
				//Stores the operations of the current relTemp as available - from SourceInterface(s)
				...
				//Stores the operations of the current relTemp as available - from TargetInterface(s)
